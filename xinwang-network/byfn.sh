...
# Generate the needed certificates, the genesis block and start the network.
function networkJoin () {
  CHANNEL_NAME=$CHANNEL_NAME TIMEOUT=$CLI_TIMEOUT DELAY=$CLI_DELAY docker-compose -f $COMPOSE_FILE_JOIN up -d 2>&1
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to start network"
    docker logs -f cli
    exit 1
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/blockchain.belink.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/blockchain.xinwang.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/blockchain.keshang.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
...
# (``peer0.blockchain.belink.com`` & ``peer0.blockchain.xinwang.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/BelinkMSPanchors.tx -channelID $CHANNEL_NAME -asOrg BelinkMSP
  if [ "$?" -ne 0 ]; then
  ...
  ./channel-artifacts/XinWangMSPanchors.tx -channelID $CHANNEL_NAME -asOrg XinWangMSP
  if [ "$?" -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######     Generating add keshang peer to channel     ##########"
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/KeShangAddChannel.tx -channelID $CHANNEL_NAME -asOrg KeShangMSP
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for Org2MSP..."
    exit 1
  fi
...
# use this as the default docker-compose-join yaml definition
...
if [ "$MODE" == "up" ]; then
  ...
  elif [ "${MODE}" == "join" ]; then ## Clear the network
  EXPMODE="Join 3rd party"
else
  printHelp
  exit 1
fi
